...
class TestWorkspace(unittest.TestCase):
    ...
    def test_add(self):
        """ Test add workspace """
        config = self.__mock_config({"workspaces": {"foo": "/foo"}})
        ws = workspace(config)
    ...
    def test_add_with_nonexistent_path(self):
        ...
        config = self.__mock_config({"workspaces": {}})
        ws = workspace(config)
        self.assertRaises(
    ...
    def test_list(self):
        ...
        ws = workspace(self.__mock_config(config_mock_data))
        list = ws.list()
